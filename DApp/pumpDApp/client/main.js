...
var getDepositAmount = function(){
  ...
  myContract.getDepositAmount(1, function(err, result){
  	if(!err){
    	...
    	if (tempDeposit.substr(1) != Session.get('deposit')) {
      	...
      	if (tempDeposit != "_0"){
	      	...
	      	Session.set('state', 'Idle');
	      	getCarAccount();
	      	//startGetDepositAmountInterval();
      	}
      }
      Session.set('deposit', tempDeposit.substr(1));
      //console.log(Session.get('deposit'));
      var tempDepositA = Session.get('deposit');
...
var startGetStateInterval = function(){
	console.log("startGetStateInterval");
	IntervalId = setInterval(function() {
	  getState();
	}, 1000);
}
...
function decideRefuelType(){
   ...
   if (radioPos == 0){
      ...
      var tempLimitGasAmount = Session.get('deposit') / Session.get('price');
...
Template.refuelGas.events({
  'click #startBtn': function(event, template){
     ...
     refueltimer = setInterval(function(){
        ...
        if(amountofgas + 1 > (Session.get('limitGasAmount'))){
        	clearInterval(refueltimer);
   	      document.getElementById("startBtn").disabled = true;
...
var calcbill = function(){
	...
	var tempDeposit = Session.get('deposit');
